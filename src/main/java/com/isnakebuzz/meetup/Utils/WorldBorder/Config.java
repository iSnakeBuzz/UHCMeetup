...
//	public static long timeUsed = 0;
    ...
    public static void showWhooshEffect(Player p) {
        if (!whooshEffect())
            return;

        p.playSound(p.getLocation(), Sound.ENDERDRAGON_HIT, 1, 2);
    }
    ...
    public static void StartBorderTimer() {
        ...
        borderTask = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new BorderCheckTask(plugin), timerTicks, timerTicks);
