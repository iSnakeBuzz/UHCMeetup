...
public class WorldFillTask implements Runnable
{
	...
	public void run()
	{
		...
		{	// notify user that task has continued automatically
			...
			sendMessage("Reminder: you can cancel at any time with \"WorldBorder fill cancel\", or pause/unpause with \"WorldBorder fill pause\".");
		}

		if (pausedForMemory)
		{	// if available memory gets too low, we automatically pause, so handle that. el meetup es mio :V, pero esta parte del codigo
			// es de badlion xd, NOpe. pero hable con un desarrollador, y me paso una parte xd. no :v
